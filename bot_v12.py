...
def fa():
 ...
 try:
  ...
  if d.get("code")==200:
   ...
   print(f"  ✅ 天行-汽车 → {len(d['newslist'])}条")
 except:pass
 return a
def fw():
 a=[]
 try:
  ...
  if d.get("code")==200:
   ...
   print(f"  ✅ 天行-微信 → {len(d['newslist'])}条")
 except:pass
 return a
def fj():
 a=[]
...
def p():
 ...
 print(f"🔍 {t().strftime('%H:%M')} 五源抓取...");al=fa()+fw()+fj()
 if not al:print("❌ 无新闻");return
 try:sn=set(open("/tmp/s").read().split())if os.path.exists("/tmp/s")else set()
 except:sn=set()
 nn=[x for x in al if x[2]not in sn]
 if not nn:print("⏹️ 无新");return
 ...
 if rr.get("code")==0:print(f"✅ {tl}条推送成功")
 else:print(f"❌ {rr}")
